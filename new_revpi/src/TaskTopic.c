...
void on_task_data_available(void* listener_data, DDS_DataReader reader) {
    ...
    task_data_t data;
    DDS_sequence_RevPiDDS_Task msgSeq          = { 0, 0, DDS_OBJECT_NIL, FALSE};
    ...
    bool is_newData = taskTopic_read(reader, &message, &data);

    if (is_newData) {
        task_data_available_callback(&data);
    }
}

void registerTaskMessageType(DDS_DomainParticipant domainParticipant, DDS_TypeSupport typeSupport) {

    char* typeName = RevPiDDS_TaskTypeSupport_get_type_name(typeSupport);

    g_status = RevPiDDS_TaskTypeSupport_register_type(typeSupport, domainParticipant, typeName);
    checkStatus(g_status, "RevPiDDS_TaskTypeSupport_register_type");

    DDS_free(typeName);

}

DDS_TopicQos* createTaskTopicQos(DDS_DomainParticipant domainParticipant) {
...
DDS_Topic taskTopic_create(DDS_DomainParticipant domainParticipant, const char* topicName) {
    ...
    DDS_Topic topic;
    char* taskTypeName = DDS_OBJECT_NIL;
    DDS_TypeSupport taskTypeSupport = DDS_OBJECT_NIL;

    // Register the Topic's type in the DDS Domain.
    taskTypeSupport = RevPiDDS_TaskTypeSupport__alloc();
    checkHandle(taskTypeSupport, "RevPiDDS_TaskTypeSupport__alloc");
    registerTaskMessageType(domainParticipant, taskTypeSupport);

    DDS_TopicQos* topicQos = createTaskTopicQos(domainParticipant);

    // Create the Topic's in the DDS Domain.
    taskTypeName = RevPiDDS_TaskTypeSupport_get_type_name(taskTypeSupport);
    topic = createTopic(domainParticipant, topicName, taskTypeName, topicQos);

    DDS_free(topicQos);
    DDS_free(taskTypeName);
    DDS_free(taskTypeSupport);
...
void taskTopic_newMessage(struct TaskMessage* message) {
    message->message_seq = DDS_sequence_RevPiDDS_Task__alloc();
    checkHandle(message->message_seq, "DDS_sequence_RevPiDDS_Task__alloc");
...
void taskTopic_write(DDS_DataWriter dataWriter, const task_data_t* data) {
    RevPiDDS_Task* taskMessage;

    taskMessage = RevPiDDS_Task__alloc();
    taskMessage->taskType = data->taskType;

    g_status = RevPiDDS_TaskDataWriter_write(dataWriter, taskMessage, DDS_HANDLE_NIL);
    checkStatus(g_status, "RevPiDDS_TaskDataWriter_write");

    DDS_free(taskMessage);
}
...
bool taskTopic_read(DDS_DataReader dataReader, struct TaskMessage* message, task_data_t* result_data) {
    ...
    g_status = RevPiDDS_TaskDataReader_read(
        dataReader,
        message->message_seq,
    ...
    checkStatus(g_status, "RevPiDDS_TaskDataReader_read");

    if (message->message_seq->_length > 0 && message->message_infoSeq->_buffer[0].valid_data) {
        result_data->taskType = message->message_seq->_buffer[0].taskType;
        is_newData = true;
    }
    RevPiDDS_TaskDataReader_return_loan(dataReader, message->message_seq, message->message_infoSeq);
...
void taskTopic_registerListener(struct DDS_DataReaderListener* listener, DDS_DataReader dataReader, on_task_data_available_t callback) {
    ...
    task_data_available_callback = callback;
}