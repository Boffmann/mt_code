...
subscriber_t subscriber_create_new(const domain_participant_t* domain_participant, const DDS_SubscriberQos* sub_qos) {
    ...
    subscriber_t new_subscriber;

    new_subscriber.dds_subscriber = DDS_DomainParticipant_create_subscriber(
        domain_participant->dds_domainParticipant,
        sub_qos,
        NULL,
        DDS_STATUS_MASK_NONE
    );
    checkHandle(new_subscriber.dds_subscriber, "DDS_DomainParticipant_create_subscriber");
...
void subscriber_dataReader_create_new(subscriber_t* subscriber, const DDS_DataReaderQos* dr_qos, const topic_t* topic) {
    ...
    subscriber->dds_dataReader = DDS_Subscriber_create_datareader(
        subscriber->dds_subscriber,
        topic->dds_topic,
        dr_qos,
        NULL,
        DDS_STATUS_MASK_NONE
    );
    checkHandle(subscriber->dds_dataReader, "DDS_Subscriber_create_datareader");
...
void subscriber_cleanup(subscriber_t* subscriber, const domain_participant_t* domain_participant) {
    ...
    status = DDS_Subscriber_delete_datareader(subscriber->dds_subscriber, subscriber->dds_dataReader);
    checkStatus(status, "DDS_Subscriber_delete_datareader");

    status = DDS_DomainParticipant_delete_subscriber(domain_participant->dds_domainParticipant, subscriber->dds_subscriber);
...
DDS_DataReaderQos* dr_qos_copy_from_topic_qos(const subscriber_t* subscriber, const topic_t* topic) {
    ...
    status = DDS_Subscriber_get_default_datareader_qos(subscriber->dds_subscriber, dataReader_qos);
    checkStatus(status, "DDS_Subscriber_get_default_datareader_qos");

    status = DDS_Subscriber_copy_from_topic_qos(subscriber->dds_subscriber, dataReader_qos, topic_qos);
